...
class TestImageMethods():
	...
	def test_decrypt_shuffle(self):
		...
		for x in range(theImage.size[0]):
			for y in range(theImage.size[1]):
				self.AssertEqual(rgb_image[x,y][0], rgb_shuffled_image[x,y][0])
                self.AssertEqual(rgb_image[x,y][1], rgb_shuffled_image[x,y][1])
                self.AssertEqual(rgb_image[x,y][2], rgb_shuffled_image[x,y][2])
